...
void main() {
  group('StubAuthRepository', () {
    test('sign-in succeeds, sets currentUser and emits on the stream',
        () async {
      final StubAuthRepository repository = StubAuthRepository();
      final List<UserEntity?> emitted = <UserEntity?>[];
      final subscription =
          repository.authStateChanges.listen(emitted.add);
    ...
    test('anonymous sign-in yields an anonymous user without email',
        () async {
      final StubAuthRepository repository = StubAuthRepository();
