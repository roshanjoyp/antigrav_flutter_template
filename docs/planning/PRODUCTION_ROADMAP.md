...
- [x] Platform flavors (2026-07-14): the Dart `AppEnv`/`AppFlavor` layer existed since Phase 1 but every build was hardcoded development. Now: `lib/app/bootstrap.dart` + `main_dev/staging/prod.dart` entry points (plain `main.dart` stays as the dev default so IDEs/desktop/web still run), Android `productFlavors` (`.dev`/`.stg` id suffixes, per-flavor launcher labels via `${appLabel}` placeholder), iOS `Debug/Release/Profile-{dev,staging,prod}` build configurations + `dev`/`staging`/`prod` schemes with suffixed bundle ids and `APP_DISPLAY_NAME` per flavor. Rename script updated for the new value homes (gradle `appLabelBase`, pbxproj display names, placeholder-aware manifest/plist updates) and verified through the generator. CI builds now pass `--flavor prod -t lib/main_prod.dart`. Also raised iOS minimum to 15.0 — cloud_firestore (Firebase iOS SDK 11+) requires it; discovered on the first real pod install
- [x] Decide license/pricing model: single-app vs. multi-app license text (current LICENSE is for the open repo; a sold template needs its own terms) (2026-07-18: **tiered Personal + Team** — Personal: one developer, unlimited End Products incl. client work; Team: one organization, unlimited developers/products. Terms in `LICENSE_COMMERCIAL.md`; the generator pipeline now writes it as the generated project's `LICENSE` (MIT never reaches buyers) with a drift guard in template_integrity_test. CodeCanyon sales stay under Envato's own license; price points deferred to the Phase 7 listings)
- [x] Demo app build (web demo or Play Store internal track) linked from the listing (2026-07-18: **web demo** — `flutter build web --base-href /demo/` from the repo root (plain `main.dart` = dev flavor on stub services, no Firebase/RevenueCat config needed), mounted at `/demo` alongside the landing page; deployment table + rebuild notes in `landing/README.md`, landing nav/hero/bottom-CTA now link to it. Build verified locally incl. boot in headless Chrome. Play internal track deliberately skipped for launch — store friction without listing value; revisit post-launch if buyers ask)
...
- [x] **SEO split (decided 2026-07-14):** Flutter web renders to canvas, so
  ...
  **static HTML landing page at `/`** (derive it from
  `docs/design/configurator_mockup.html`, which already implements the full
  design in crawlable HTML) with proper meta/OG/structured-data tags, and
  mount the **Flutter configurator at `/configure`** linked from the
